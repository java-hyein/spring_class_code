...
public class PageMaker {
	private Criteria cri; // page, perPageNum을 가지고 있다 (변수 4개갖고있음)
							// 한 개의 페이지에 보여줄 변수들
	...
	private boolean prev; // << 버튼 출력 여부 이전 버튼을 누를 수 있는 경우/없는 경우 분류를 위함
	private boolean next; // >> 버튼 출력 여부
	...
	public void setTotalCount(int totalCount) {
		...
		calcData(); // TotalCount가 set 될때 10개들을 설정한다. 전체 필드 변수들 세팅 : 전체 게시글 수의 setter가 호출될 때 전체
					// 세팅되도록 함
	}

	private void calcData() { // 전체 변수 값들을 계산하는 메서드
		...
		if (endPage > tempEndPage) {
			endPage = tempEndPage;
			//endPage가 60 이고 tempEndPage가 52이라고 하면 endpage의 값을 tempPage로 바꿔준다.
			//결국엔 endpage가 52가 되는거 
		}
	...
	public String makeQuery(int page) {
		UriComponents uriComponentsBuilder = UriComponentsBuilder.newInstance().queryParam("page", page) // page = 3
				...
				.build(); // ?page=3&perPageNum=10 <== get방식에 대한 key,value
						  // 게시판 아래 번호에 링크인데 30번 부터 40번의 쿼리를 가져온다 
